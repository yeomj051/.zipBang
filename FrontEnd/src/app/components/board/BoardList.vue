<template>
  <b-container class="bv-example-row mt-3">
    ...
    <div data-aos="fade-up">
      <b-row>
        <b-col class="text-center my-5" cols="12" v-if="itemsForCreatorsList.length === 0">
        ...
        <b-col v-if="itemsForCreatorsList.length > 0">
          <div class="question__section mx-auto">
            <div class="question-item bg-dark text-light w-100 px-3 px-md-4 py-1 my-4 row" duration="2000">
              <!-- <b-icon icon="question-circle" class="ml-1 ml-md-3 mr-3 mb-0"></b-icon> -->
              <div class="col-1"><p class="my-2 font-weight-bold text-left">글번호</p></div>
            ...
            <div v-if="boardList && boardList.length != 0">
              <board-list-item v-for="(board, index) in boardList" :key="index" :board="board" />
            </div>
          </div>
        </b-col>
      </b-row>
...
export default {
  ...
  computed: {
    ...
    itemsForCreatorsList() {
      return this.boardList.slice((this.currentPage - 1) * this.ListsperPage, this.currentPage * this.ListsperPage);
    },
  },
  created() {
    const params = {};
